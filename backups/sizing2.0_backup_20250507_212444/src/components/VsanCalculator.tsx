...
const VsanCalculator = () => {
  ...
  useEffect(() => {
    const fetchDiskOptions = async () => {
      try {
        ...
        if (data) {
          setDiskOptions(data);
          // Set initial disk config based on first available disk
          if (data.length > 0) {
            const firstDisk = data[0];
            setVsanConfig(prev => ({
              ...prev,
              diskConfig: {
                type: firstDisk.type as 'SSD' | 'NVMe' | 'NL-SAS',
                formFactor: firstDisk.form_factor as '2.5' | '3.5',
                count: getMaxDisksByFormFactor(prev.formFactor, firstDisk.form_factor as '2.5' | '3.5')
              }
            }));
          }
        }
      } catch (error) {
        console.error('Error in fetchDiskOptions:', error);
      }
    };
  ...
  const getMaxDisksByFormFactor = useCallback((formFactor: '1U' | '2U', diskType: '2.5' | '3.5') => {
    if (formFactor === '1U') {
      if (diskType === '2.5') {
        return 12; // 12x 2.5" disks in 1U
      } else {
        return 4; // 4x 3.5" disks in 1U
      }
    } else { // 2U
      if (diskType === '2.5') {
        return 24; // 24x 2.5" disks in 2U
      } else {
        return 12; // 12x 3.5" disks in 2U
      }
    }
  }, []);
  ...
  const calculateUsableCapacity = useCallback((hosts: number) => {
    const selectedDisk = diskOptions.find(d => 
      d.type === vsanConfig.diskConfig.type && 
      d.form_factor === vsanConfig.diskConfig.formFactor
    );
        ...
        <div className="space-y-6">
            ...
            >
              ...
              <option value="2U">2U Server</option>
                ...
                <div>
            <label className="block text-sm font-medium mb-2">Disk Type</label>
            <select
              value={vsanConfig.diskConfig.type}
              onChange={(e) => {
                const newType = e.target.value as 'SSD' | 'NVMe' | 'NL-SAS';
                const newFormFactor = newType === 'NL-SAS' ? '3.5' : '2.5';
                const maxDisks = getMaxDisksByFormFactor(vsanConfig.formFactor, newFormFactor);
                setVsanConfig(prev => ({
                  ...prev,
                  diskConfig: {
                    type: newType,
            ...
            >
              ...
              <option value="NL-SAS">NL-SAS</option>
                ...
                <div>
            <label className="block text-sm font-medium mb-2">Disk Capacity</label>
                  <select
              value={vsanConfig.diskConfig.count}
                    onChange={(e) => {
            ...
            >
              {diskOptions
                .filter(d => d.type === vsanConfig.diskConfig.type && d.form_factor === vsanConfig.diskConfig.formFactor)
                .map(disk => (
                  <option key={disk.id} value={disk.capacity_gb}>
                    {formatStorage(disk.capacity_gb)}
                      </option>
                    ))}
                  </select>
                </div>
          ...
          <div>
            <label className="block text-sm font-medium mb-2">Number of Disks</label>
                    <input
              type="number"
              value={vsanConfig.diskConfig.count}
              onChange={(e) => {
                const newCount = Number(e.target.value);
                const maxDisks = getMaxDisksByFormFactor(vsanConfig.formFactor, vsanConfig.diskConfig.formFactor);
                setVsanConfig(prev => ({
                  ...prev,
                  diskConfig: {
                    ...prev.diskConfig,
                    count: Math.min(newCount, maxDisks)
                  }
                }));
              }}
              min="1"
              max={getMaxDisksByFormFactor(vsanConfig.formFactor, vsanConfig.diskConfig.formFactor)}
              className="w-full bg-slate-700/50 border border-slate-600 rounded px-3 py-2"
            />
            <p className="text-sm text-slate-400 mt-1">
              Maximum {getMaxDisksByFormFactor(vsanConfig.formFactor, vsanConfig.diskConfig.formFactor)} disks for {vsanConfig.formFactor} server
            </p>
                  </div>
