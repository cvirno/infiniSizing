...
function SAPSizing() {
  ...
  const handleInputChange = (field: string, value: string | number) => {
    setInputs(prev => {
      ...
      if (newInputs.prodSaps > 0 || newInputs.prodMemory > 0 ||
          newInputs.qaSaps > 0 || newInputs.qaMemory > 0 ||
          newInputs.devSaps > 0 || newInputs.devMemory > 0 ||
          newInputs.appSaps > 0 || newInputs.appMemory > 0) {
        ...
        if (combineQaDev) {
          // Se combinar QA/DEV, soma os valores e calcula uma única configuração
          ...
          setResults({ production: prod, qa: qaDev, dev: qaDev, applications: app });
        } else {
          // Se não combinar, calcula configurações separadas
          const qa = calculateSizingBySAPS(newInputs.qaSaps, newInputs.qaMemory);
          const dev = calculateSizingBySAPS(newInputs.devSaps, newInputs.devMemory);
          setResults({ production: prod, qa, dev, applications: app });
        }
      } else {
        // Se não houver dados de entrada, limpa os resultados
        setResults(null);
      }
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div className="space-y-2">
                        <p className="text-zinc-300">
                          <span className="text-zinc-400">Processor:</span>
                          <br />
                          <span className="text-white font-medium">{results?.dev.processor.model || '-'}</span>
                        </p>
                        <p className="text-zinc-300">
                          <span className="text-zinc-400">Cores:</span>
                          <br />
                          <span className="text-white font-medium">
                            {calculateTotalCores(results?.dev?.processor, results?.dev?.sockets)}
                          </span>
                        </p>
                        <p className="text-zinc-300">
                          <span className="text-zinc-400">Sockets:</span>
                          <br />
                          <span className="text-white font-medium">{results?.dev.sockets || '-'}</span>
                        </p>
                      </div>
                      <div className="space-y-2">
                        <p className="text-zinc-300">
                          <span className="text-zinc-400">SAPS:</span>
                          <br />
                          <span className="text-white font-medium">{results?.dev.saps.toLocaleString() || '-'}</span>
                        </p>
                        <p className="text-zinc-300">
                          <span className="text-zinc-400">Memory:</span>
                          <br />
                          <span className="text-white font-medium">{results?.dev.memory || '-'} GB</span>
                        </p>
                        <p className="text-zinc-300">
                          <span className="text-zinc-400">TDP:</span>
                          <br />
                          <span className="text-white font-medium">{results?.dev.processor.tdp || '-'}W</span>
                        </p>
                      </div>
                    </div>
                  </div>
