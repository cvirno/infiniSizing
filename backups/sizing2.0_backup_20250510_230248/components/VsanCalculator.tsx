...
const VsanCalculator = () => {
  ...
  const getMaxDisksByFormFactor = (formFactor: '1U' | '2U', storageType: 'all-flash' | 'hybrid') => {
    if (storageType === 'all-flash') {
      return {
        smallDiskCount: formFactor === '1U' ? 12 : 24,
        largeDiskCount: 0
      };
    } else {
      ...
      if (formFactor === '1U') {
        return {
          smallDiskCount: 2, // Fixo em 2 discos de 2.5"
          largeDiskCount: 4  // Fixo em 4 discos de 3.5"
        };
      } else {
        return {
          smallDiskCount: 4,  // Fixo em 4 discos de 2.5"
          largeDiskCount: 12  // Fixo em 12 discos de 3.5"
        };
      }
    }
  };

  // Atualizar a função de inicialização do primeiro disco
  useEffect(() => {
    if (diskOptions.length > 0) {
      const firstSSD = diskOptions.find(d => d.type === 'SSD' && d.form_factor === '2.5');
      if (firstSSD) {
        const maxDisks = getMaxDisksByFormFactor(vsanConfig.formFactor, vsanConfig.storageType);
        setVsanConfig(prev => ({
          ...prev,
          diskConfig: {
            type: 'SSD',
            ...
            smallDiskCount: maxDisks.smallDiskCount,
            largeDiskCount: maxDisks.largeDiskCount
          }
        }));
      }
    }
  ...
  const handleStorageTypeChange = (type: 'all-flash' | 'hybrid') => {
    const maxDisks = getMaxDisksByFormFactor(vsanConfig.formFactor, type);
    setVsanConfig(prev => ({
      ...prev,
      storageType: type,
      diskConfig: {
        ...prev.diskConfig,
        type: type === 'hybrid' ? 'NL-SAS' : 'SSD',
        formFactor: type === 'hybrid' ? '3.5' : '2.5',
        smallDiskCount: maxDisks.smallDiskCount,
        largeDiskCount: maxDisks.largeDiskCount
      }
    }));
  ...
  const handleFormFactorChange = (newFormFactor: '1U' | '2U') => {
    const maxDisks = getMaxDisksByFormFactor(newFormFactor, vsanConfig.storageType);
    setVsanConfig(prev => ({
      ...prev,
  ...
  const calculateUsableCapacity = useCallback((hosts: number) => {
    ...
    const capacityPerHost = selectedDisk.capacity_gb * (vsanConfig.diskConfig.smallDiskCount + vsanConfig.diskConfig.largeDiskCount);
  ...
  const handleDiskTypeChange = (type: 'SSD' | 'NVMe' | 'NL-SAS') => {
    const maxDisks = getMaxDisksByFormFactor(vsanConfig.formFactor, vsanConfig.storageType);
    setVsanConfig(prev => ({
      ...prev,
      diskConfig: {
        ...prev.diskConfig,
        type,
        formFactor: type === 'NL-SAS' ? '3.5' : '2.5',
        smallDiskCount: maxDisks.smallDiskCount,
        largeDiskCount: maxDisks.largeDiskCount
      }
    }));
  ...
  const handleDiskCountChange = (count: number) => {
    ...
    setVsanConfig(prev => ({
      ...
      diskConfig: {
        ...prev.diskConfig,
        smallDiskCount: Math.min(count, maxDisks.smallDiskCount)
      }
    }));
  };

  const handleLargeDiskCountChange = (count: number) => {
    const maxDisks = getMaxDisksByFormFactor(vsanConfig.formFactor, vsanConfig.storageType);
    setVsanConfig(prev => ({
      ...prev,
      diskConfig: {
        ...prev.diskConfig,
        largeDiskCount: Math.min(count, maxDisks.largeDiskCount)
      }
    }));
  };
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) => {
                ...
                handleFormFactorChange(newFormFactor);
              }}
            ...
            <div>
              ...
              <div className="flex gap-4">
                ...
                >
                  ...
                  {diskOptions
                    .filter(d => (d.type === 'SSD' || d.type === 'NVMe') && d.form_factor === '2.5')
                    .map(disk => (
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    setVsanConfig(prev => ({
                      ...
                      diskConfig: {
                        ...prev.diskConfig,
                        smallDiskCount: Math.min(newCount, maxDisks.smallDiskCount)
                      }
                    }));
                  }}
                  min={vsanConfig.storageType === 'hybrid' ? (vsanConfig.formFactor === '1U' ? 2 : 4) : (vsanConfig.formFactor === '1U' ? 12 : 24)}
                  max={vsanConfig.storageType === 'hybrid' ? (vsanConfig.formFactor === '1U' ? 2 : 4) : (vsanConfig.formFactor === '1U' ? 12 : 24)}
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      setVsanConfig(prev => ({
                        ...
                        diskConfig: {
                          ...prev.diskConfig,
                          largeDiskCount: Math.min(newCount, maxDisks.largeDiskCount)
                        }
                      }));
                    }}
                    min={vsanConfig.formFactor === '1U' ? 4 : 12}
