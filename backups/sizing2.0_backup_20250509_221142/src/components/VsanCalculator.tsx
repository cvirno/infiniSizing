          ...
          <div>
            ...
            <label className="block text-sm font-medium mb-2">Memory per Host (GB)</label>
              ...
              max="2048"
                        ...
                        <div>
                          <label className="block text-sm font-medium mb-2">Core Ratio (vCPU:pCPU)</label>
                          <select
                            value={vsanConfig.coreRatio}
                            onChange={(e) => setVsanConfig({ ...vsanConfig, coreRatio: Number(e.target.value) })}
                            className="w-full bg-slate-700/50 border border-slate-600 rounded px-3 py-2"
                          >
                          ...
                          <p className="mt-1 text-xs text-slate-400">
                            {vsanConfig.coreRatio <= 3 ? 'Recommended for CPU-intensive workloads' :
                             vsanConfig.coreRatio <= 4 ? 'Recommended for general purpose workloads' :
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="bg-slate-800/50 backdrop-blur-sm p-6 rounded-xl">
            ...
            <div className="mt-6 pt-6 border-t border-slate-700">
              <h3 className="text-lg font-semibold mb-4">Resource Visualization</h3>
              ...
              <div className="mb-6">
                <div className="flex justify-between items-center mb-2">
                  ...
                  <span className="text-sm text-slate-400">
                    Core Ratio: {vsanConfig.coreRatio}:1
                  </span>
                </div>
                <div className="bg-slate-700/30 p-4 rounded-lg">
                  <div className="flex justify-between mb-2">
                    <span className="text-sm">Physical Cores: {vsanConfig.coresPerSocket * vsanConfig.socketsPerHost * recommendation.minimumHosts}</span>
                    <span className="text-sm">Available vCPUs: {vsanConfig.coresPerSocket * vsanConfig.socketsPerHost * recommendation.minimumHosts * vsanConfig.coreRatio}</span>
                  </div>
                  <div className="h-6 bg-slate-600 rounded-full overflow-hidden">
                    <div 
                      className="h-full bg-blue-500 rounded-full transition-all duration-300"
                      style={{ width: `${Math.min(100, (totalResources.vCPUs / (vsanConfig.coresPerSocket * vsanConfig.socketsPerHost * recommendation.minimumHosts * vsanConfig.coreRatio)) * 100)}%` }}
                    />
                  </div>
                  <div className="mt-2 flex justify-between text-xs text-slate-400">
                    <span>Used: {totalResources.vCPUs} vCPUs</span>
                    <span>Available: {vsanConfig.coresPerSocket * vsanConfig.socketsPerHost * recommendation.minimumHosts * vsanConfig.coreRatio} vCPUs</span>
                  </div>
                </div>
              </div>
              ...
              <div className="mb-6">
                ...
                <div className="bg-slate-700/30 p-4 rounded-lg">
                  <div className="flex justify-between mb-2">
                    <span className="text-sm">Total Memory: {vsanConfig.memoryPerHost * recommendation.minimumHosts} GB</span>
                  </div>
                  <div className="h-6 bg-slate-600 rounded-full overflow-hidden">
                    <div 
                      className="h-full bg-green-500 rounded-full transition-all duration-300"
                      style={{ width: `${Math.min(100, (totalResources.memory / (vsanConfig.memoryPerHost * recommendation.minimumHosts)) * 100)}%` }}
                    />
                  </div>
                  <div className="mt-2 flex justify-between text-xs text-slate-400">
                    <span>Used: {totalResources.memory} GB</span>
                    <span>Available: {vsanConfig.memoryPerHost * recommendation.minimumHosts} GB</span>
                  </div>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="bg-slate-700/30 p-4 rounded-lg">
                  <div className="flex justify-between mb-2">
                    <span className="text-sm">Total Storage: {formatStorage(calculateUsableCapacity(recommendation.minimumHosts))}</span>
                  </div>
                  <div className="h-6 bg-slate-600 rounded-full overflow-hidden">
                    <div 
                  ...
                  <div className="mt-2 flex justify-between text-xs text-slate-400">
                    <span>Used: {formatStorage(totalResources.storage * recommendation.storageOverhead.multiplier)}</span>
                    <span>Available: {formatStorage(calculateUsableCapacity(recommendation.minimumHosts))}</span>
                  </div>
                </div>
              </div>
            </div>
